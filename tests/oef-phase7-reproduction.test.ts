...
describe("Phase 7 pinned reproduction", () => {
  ...
  test("uses the explicit Phase 2 replay port for deterministic 5/5 evidence", async () => {
    ...
    const adapter = createDeterministicPhase2ReplayAdapter({ outcomes: [true, true, true, true, true] });
    const first = await runPinnedReproduction(manifest, adapter);
    const second = await runPinnedReproduction(manifest, adapter);
