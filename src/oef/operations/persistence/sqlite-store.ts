...
import { assertCausalTime, assertOperationOwner, assertOperationScope, assertOperationTimestamp, assertOperationTransition, canonicalEffectDescriptor, jobStateSchema, leaseExpiry, parseOperationFailure, parseRetryPolicy, retryAvailableAt, serializeOperationPayload, type CancellationCapabilityVerifier, type EffectReceipt, type FailureDetails, type JsonValue, type OperationAttempt, type OperationJob, type RetryPolicy } from "../core";
...
export class SqliteOperationsStore {
  ...
  enqueue(input: { scope_id: string; kind: string; idempotency_key: string; payload: Record<string, unknown>; priority: number; max_attempts: number; retry_policy: RetryPolicy; now: string }): OperationJob {
    ...
    return this.transaction(() => {
      ...
      this.database.query(`INSERT INTO operation_jobs (job_id, scope_id, kind, idempotency_key, payload_json, payload_hash, priority, max_attempts, retry_policy_json, attempt_count, state, available_at, lease_owner, lease_expires_at, lease_acquired_at, run_started_at, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, 'PENDING', ?, NULL, NULL, NULL, NULL, ?, ?)`).run(jobId, input.scope_id, input.kind, input.idempotency_key, payload.json, canonicalSha256(payload.value), input.priority, input.max_attempts, JSON.stringify(retryPolicy), input.now, input.now, input.now);
  ...
  succeed(input: { scope_id: string; job_id: string; owner: string; effect: Record<string, unknown>; now: string }): OperationJob {
    ...
    return this.transaction(() => {
      ...
      this.database.query("UPDATE operation_jobs SET state='SUCCEEDED', attempt_count=?, lease_owner=NULL, lease_expires_at=NULL, updated_at=? WHERE scope_id=? AND job_id=?").run(attempt.attempt_number, input.now, input.scope_id, job.job_id); return this.getRequired(input.scope_id, job.job_id);
    });
  }
  fail(input: { scope_id: string; job_id: string; owner: string; failure: FailureDetails; retryable?: boolean; now: string }): OperationJob {
    ...
    return this.transaction(() => { const job = this.requireActiveLease(input.scope_id, input.job_id, input.owner, input.now); assertCausalTime(input.now, job.updated_at); if (job.state !== "RUNNING") throw new Error("OPERATION_STATE_TRANSITION_INVALID"); if (input.retryable === false) { const attempt = this.recordAttempt(job, input.owner, input.owner, "FAILED", failure, input.now); assertOperationTransition(job.state, "FAILED"); this.database.query("UPDATE operation_jobs SET state='FAILED', attempt_count=?, lease_owner=NULL, lease_expires_at=NULL, updated_at=? WHERE scope_id=? AND job_id=?").run(attempt.attempt_number, input.now, job.scope_id, job.job_id); return this.getRequired(job.scope_id, job.job_id); } return this.finishFailedAttempt(job, input.owner, failure, input.now, "FAILED"); });
  }
  cancel(input: { scope_id: string; job_id: string; owner?: string; capability?: string; actor?: string; now: string }): OperationJob {
    ...
    return this.transaction(() => {
      ...
      if (job.state === "LEASED" || job.state === "RUNNING") {
        ...
        this.database.query("UPDATE operation_jobs SET state='CANCELLED', attempt_count=?, lease_owner=NULL, lease_expires_at=NULL, updated_at=? WHERE scope_id=? AND job_id=?").run(attempt.attempt_number, input.now, input.scope_id, job.job_id);
      } else {
        this.verifyCancellationCapability(input, job.job_id);
        assertOperationTransition(job.state, "CANCELLED"); this.database.query("UPDATE operation_jobs SET state='CANCELLED', lease_owner=NULL, lease_expires_at=NULL, updated_at=? WHERE scope_id=? AND job_id=?").run(input.now, input.scope_id, job.job_id);
      }
  ...
  private decodeJob(row: unknown): OperationJob | null {
    ...
    try {
      ...
      if (payload.json !== source.payload_json || canonicalSha256(payload.value) !== source.payload_hash || JSON.stringify(retryPolicy) !== source.retry_policy_json || !jobStateSchema.safeParse(source.state).success) throw new Error("tampered");
  ...
  private decodeAttempt(row: Record<string, unknown>, scopeId: string, jobId: string): OperationAttempt {
    try { const payloadJson = String(row.payload_json); const raw = JSON.parse(payloadJson) as OperationAttempt; const snapshot = serializeOperationPayload(raw); if (snapshot.json !== payloadJson || raw.scope_id !== scopeId || raw.job_id !== jobId || raw.attempt_id !== row.attempt_id || raw.scope_id !== row.scope_id || raw.job_id !== row.job_id || raw.attempt_number !== row.attempt_number || raw.owner !== row.owner || raw.actor !== row.actor || raw.outcome !== row.outcome || raw.started_at !== row.started_at || raw.finished_at !== row.finished_at || JSON.stringify(raw.failure) !== (row.failure_json === null ? "null" : row.failure_json) || !Number.isSafeInteger(raw.attempt_number) || raw.attempt_number < 1) throw new Error("tampered"); assertOperationOwner(raw.owner); assertOperationOwner(raw.actor); assertOperationTimestamp(raw.started_at); assertOperationTimestamp(raw.finished_at); if (Date.parse(raw.finished_at) < Date.parse(raw.started_at) || !["SUCCEEDED", "FAILED", "LEASE_EXPIRED", "CANCELLED"].includes(raw.outcome)) throw new Error("tampered"); if (raw.failure !== null) parseOperationFailure(raw.failure); return raw; } catch { throw new Error("OPERATION_PERSISTENCE_TAMPERED"); }
  }
  private decodeReceipt(row: Record<string, unknown>, scopeId: string, jobId: string): EffectReceipt {
    try { const payloadJson = String(row.payload_json); const raw = JSON.parse(payloadJson) as EffectReceipt; const snapshot = serializeOperationPayload(raw); const effect = canonicalEffectDescriptor(raw.effect); if (snapshot.json !== payloadJson || raw.scope_id !== scopeId || raw.job_id !== jobId || raw.scope_id !== row.scope_id || raw.job_id !== row.job_id || raw.effect_hash !== row.effect_hash || raw.effect_hash !== effect.effect_hash || effect.effect_json !== row.effect_json || raw.succeeded_at !== row.succeeded_at) throw new Error("tampered"); assertOperationTimestamp(raw.succeeded_at); return raw; } catch { throw new Error("OPERATION_PERSISTENCE_TAMPERED"); }
  }
}
