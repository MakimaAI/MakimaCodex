...
export class SqliteOperationsStore {
  ...
  private decodeJob(row: unknown): OperationJob | null {
    ...
    try {
      ...
      assertOperationScope(source.scope_id); if (!Number.isSafeInteger(source.priority) || !Number.isSafeInteger(source.max_attempts) || source.max_attempts < 1 || source.max_attempts > 30 || !Number.isSafeInteger(source.attempt_count) || source.attempt_count < 0 || source.attempt_count > source.max_attempts || !source.job_id || !source.kind || !source.idempotency_key) throw new Error("tampered"); if (source.lease_owner !== null) assertOperationOwner(source.lease_owner); if (Date.parse(source.updated_at) < Date.parse(source.created_at) || (source.run_started_at && source.lease_acquired_at && Date.parse(source.run_started_at) < Date.parse(source.lease_acquired_at))) throw new Error("tampered");
      const active = source.state === "LEASED" || source.state === "RUNNING"; if (active !== (source.lease_owner !== null && source.lease_expires_at !== null && source.lease_acquired_at !== null) || (source.state === "RUNNING" && source.run_started_at === null) || (!active && (source.lease_owner !== null || source.lease_expires_at !== null || source.lease_acquired_at !== null || source.run_started_at !== null))) throw new Error("tampered");
