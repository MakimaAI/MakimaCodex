...
export class SqliteOperationsStore {
  ...
  private decodeJob(row: unknown): OperationJob | null {
    ...
    try {
      ...
      if (active && (Date.parse(source.lease_acquired_at!) < Date.parse(source.created_at) || Date.parse(source.lease_acquired_at!) > Date.parse(source.updated_at) || Date.parse(source.lease_expires_at!) <= Date.parse(source.updated_at) || (source.state === "RUNNING" && (Date.parse(source.run_started_at!) < Date.parse(source.lease_acquired_at!) || Date.parse(source.run_started_at!) > Date.parse(source.updated_at))))) throw new Error("tampered");
