...
const temporary = mkdtempSync(join(workRoot, "oef-phase3-wide-coverage-"));
...
try {
  const tests = readdirSync(join(repositoryRoot, "tests"))
    .filter(name => name.startsWith("oef-phase3-") && name.endsWith(".test.ts"))
    .sort().map(name => join(repositoryRoot, "tests", name));
  ...
  const childEntrypoints = new Set([
    "src/oef/phase3/acceptance/runtime-worker.ts",
    "src/oef/phase3/acceptance/review-worker.ts",
  ]);
  const expected = [...files(join(repositoryRoot, "src", "oef", "phase3")), "src/cli/oef-phase3.ts"]
    .filter(path => !childEntrypoints.has(path)).sort();
  const missing = expected.filter(path => !coverage.has(path));
  if (missing.length > 0) throw new Error(`Phase 3 coverage is missing production files:\n${missing.join("\n")}`);
  const totals = expected.reduce<Totals>((sum, path) => {
    const value = coverage.get(path)!;
    sum.functionsFound += value.functionsFound; sum.functionsHit += value.functionsHit;
    sum.linesFound += value.linesFound; sum.linesHit += value.linesHit;
    return sum;
  ...
  console.log(`Phase 3 wide coverage: functions ${functions.toFixed(2)}%, lines ${lines.toFixed(2)}%`);
  if (functions < 90 || lines < 90) throw new Error("Phase 3 wide coverage must remain at or above 90% for functions and lines.");
} finally {
